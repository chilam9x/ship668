...
@section('content')
    ...
    <section class="banner-sub">
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-8">
                    <h1>Đăng ký làm đại lý thu gom</h1>
                    <span><a href="{!! url('/') !!}">Trang chủ</a> / <b>Đăng ký làm đại lý thu gom</b> </span>
                </div>
                <div class="col-md-2"></div>
            </div>
        </div>
    ...
    <section class="sub-content">
        {{ Form::open(['url' => 'front-ent/agency', 'method' => 'post', 'enctype' => 'multipart/form-data']) }}
        <div class="container">
                            ...
                            <div class="row" style="margin-top: 15px">
                                ...
                                <div class="col-lg-6" style="padding-right: 0px !important;">
                                    <div class="form-group">
                                        ...
                                        <input style="padding: 6px 10px; width: 100% !important;" name="home_number"
                ...
                <div class="col-md-12 col-sm-12">
                    <ul>
                        <li>
                            ...
                            <input name="agency_name" type="text" value="{!! old( 'agency_name') !!}"
                        ...
                        <li>
                            <label>Hotline:</label>
                            <input name="hot_line" type="text" value="{!! old( 'hot_line') !!}"
                                   placeholder="Nhập đường dây nóng"/>
                            @if ($errors->has('hot_line'))
                                @foreach ($errors->get('hot_line') as $error)
                                    <div style="width: 70%" class="pull-right">
                                        <span style="color: red;" class="help-block">{!! $error !!}</span>
                                    </div>
                                @endforeach
                        ...
                        <li>
                            ...
                            <div class="row">
                                <div class="col-lg-6" style="padding-left: 0px !important;">
                                    <div class="form-group">
                                        <label style="width: 100%">Tỉnh/Thành phố</label>
                                        {{ Form::select('agency_province', \App\Models\Province::getProvinceOption() , old('agency_province'),
                                         ['class' => 'form-control', 'style' => 'width:100%', 'id'=>'agency_province', 'onchange'=>'loadDistrictAgency()']) }}
                                    </div>
                                </div>
                                <div class="col-lg-6" style="padding-right: 0px !important;">
                                    <div class="form-group">
                                        <label style="width: 100%">Quận/Huyện</label>
                                        <select style="width:100% !important;" id="agency_district"
                                                onchange="loadWardAgency(this.value)" name="agency_district"
                                                class="form-control">
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row" style="margin-top: 15px">
                                <div class="col-lg-6" style="padding-left: 0px !important;">
                                    <div class="form-group">
                                        <label style="width: 100%">Xã/Phường</label>
                                        <select style="width:100% !important;" id="agency_ward" name="agency_ward"
                                                class="form-control">
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-6" style="padding-right: 0px !important;">
                                    <div class="form-group">
                                        <label style="width: 100%">Số nhà / tên đường</label>
                                        <input style="padding: 6px 10px; width: 100% !important;" name="agency_home_number"
                                               class="form-control spinner" value="{!! old( 'agency_home_number') !!}" type="text"
                                               placeholder="Nhập số nhà">
                                        @if ($errors->has('agency_home_number'))
                                            @foreach ($errors->get('agency_home_number') as $error)
                                                <div style="width: 100%" class="pull-right">
                                                    <span style="color: red;" class="help-block">{!! $error !!}</span>
                                                </div>
                                            @endforeach
                        ...
                        <li>
                            <label></label>
                            <button type="submit" id="myBtn">Đăng ký</button>
...
@push('script')
    <script>
        ...
        function loadDistrictAgency() {
            var province = $('#agency_province').val();
            $("#agency_district option[value!='-1']").remove();
            $.ajax({
                type: "GET",
                url: '{{url('/ajax/get_district/')}}/' + province
            }).done(function (msg) {
                var i;
                for (i = 0; i < msg.length; i++) {
                    if (msg[i]['id'] == '{{old('agency_district')}}') {
                        $('select[name="agency_district"]').append('<option value="' + msg[i]['id'] + '" selected>' + msg[i]['name'] + '</option>')
                    } else {
                        $('select[name="agency_district"]').append('<option value="' + msg[i]['id'] + '">' + msg[i]['name'] + '</option>')
                    }
                }
                if ("{{old('agency_district')}}") {
                    loadWardAgency('{{old('agency_district')}}');
                } else {
                    loadWardAgency(msg[0]['id']);

                }
            });
        }

        function loadWardAgency(id) {
            $("#agency_ward option[value!='-1']").remove();
            $.ajax({
                type: "GET",
                url: '{{url('/ajax/get_ward/')}}/' + id
            }).done(function (msg) {
                var i;
                for (i = 0; i < msg.length; i++) {
                    if (msg[i]['id'] == '{{old('agency_ward')}}') {
                        $('select[name="agency_ward"]').append('<option value="' + msg[i]['id'] + '" selected>' + msg[i]['name'] + '</option>')
                    } else {
                        $('select[name="agency_ward"]').append('<option value="' + msg[i]['id'] + '">' + msg[i]['name'] + '</option>')
                    }
                }
            });
